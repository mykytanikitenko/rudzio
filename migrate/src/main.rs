...
// This crate is a single-binary tool; module items are visible only
// within it, so `unreachable_pub` would require `pub(crate)` on every
// re-used API. We keep `pub fn` for readability and scope the lint here.
// `dead_code` is allowed because some helper fns land ahead of their
// caller as the modules are built out in passes.
#![allow(
    unreachable_pub,
    dead_code,
...
fn run(args: &cli::Cli) -> anyhow::Result<ExitCode> {
    let stdin = io::stdin();
        ...
        if want_shared_runner && !args.dry_run && pkg_had_conversions {
            ...
            match runner_scaffold::ensure_tests_main(&tests_main, Some(&crate_lib_name)) {
                Ok(runner_scaffold::ScaffoldOutcome::Created) => {
                    ...
                    // A fresh tests/main.rs needs its [[test]] entry so
                    // cargo test actually invokes the rudzio runner.
                    pkg_edits
                        .tests_integration
...
fn is_under_tests_dir(file: &std::path::Path, pkg_root: &std::path::Path) -> bool {
