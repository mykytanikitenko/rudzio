...
mod tests {
    ...
    fn direct_git_declaration_produces_git_location() -> anyhow::Result<()> {
        ...
        anyhow::ensure!(
            spec.features
                .iter()
                .any(|f| f == "runtime-tokio-multi-thread")
        );
        Ok(())
    }
    ...
    fn warning_message_includes_actionable_fix() -> anyhow::Result<()> {
        ...
        anyhow::ensure!(
            warnings
                .iter()
                .any(|w| w.contains("#[cfg(any(test, rudzio_test))]")),
    ...
    fn write_bridge_crate_bridge_cargo_toml_uses_relative_lib_path() -> anyhow::Result<()> {
        ...
        let rendered = fs::read_to_string(out_dir.join(bridge_dir_name(&m)).join("Cargo.toml"))?;
        anyhow::ensure!(
    ...
    fn restrict_to_paths_errors_when_no_matches() -> anyhow::Result<()> {
        ...
        let err = plan
            .restrict_to_paths(&[unrelated_dir.clone()])
            .unwrap_err();
        anyhow::ensure!(
